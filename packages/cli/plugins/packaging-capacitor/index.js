...
const packagingCapacitor = () => async ({ logger, config, cwd }) => {
...
async function checkCapacitorInstalled(logger) {
    try {
...
async function initializeCapacitor(cwd, config, logger) {
...
async function syncWebAssets(cwd, config, logger) {
...
async function addAndroidPlatform(cwd, logger) {
...
async function updateAndroidConfig(cwd, config, logger) {
    ...
    if (fs.existsSync(androidManifestPath)) {
        ...
        if (config.android.targetSdk) {
            ...
            if (fs.existsSync(buildGradlePath)) {
                ...
                buildGradle = buildGradle.replace(/compileSdkVersion \d+/, `compileSdkVersion ${config.android.targetSdk}`);
                buildGradle = buildGradle.replace(/targetSdkVersion \d+/, `targetSdkVersion ${config.android.targetSdk}`);
        ...
        if (config.android.version) {
            ...
            if (fs.existsSync(buildGradlePath)) {
                let buildGradle = fs.readFileSync(buildGradlePath, 'utf8');
                buildGradle = buildGradle.replace(/versionCode \d+/, `versionCode ${config.android.version.code}`);
                buildGradle = buildGradle.replace(/versionName "[^"]*"/, `versionName "${config.android.version.name}"`);
                fs.writeFileSync(buildGradlePath, buildGradle);
...
<network-security-config>
        ...
        <trust-anchors>
            ...
            if (fs.existsSync(manifestPath)) {
                ...
                if (!manifestContent.includes('android:networkSecurityConfig')) {
                    manifestContent = manifestContent.replace(/<application([^>]*)>/, '<application$1\n        android:networkSecurityConfig="@xml/network_security_config"\n        android:usesCleartextTraffic="true">');
                }
    ...
    <uses-permission android:name="android.permission.VIBRATE" />`;
                    manifestContent = manifestContent.replace(/<\/manifest>/, `${additionalPermissions}\n<\/manifest>`);
                }
    ...
    compileOptions {
        ...
        targetCompatibility JavaVersion.VERSION_21
                        ...
                        buildGradle = buildGradle.replace(/(\s+)(}\s*$)/m, `$1${javaConfig}$1$2`);
                    }
                    else {
                        // Update existing Java version to 21
                        buildGradle = buildGradle.replace(/sourceCompatibility JavaVersion\.VERSION_\d+/, 'sourceCompatibility JavaVersion.VERSION_21');
                        buildGradle = buildGradle.replace(/targetCompatibility JavaVersion\.VERSION_\d+/, 'targetCompatibility JavaVersion.VERSION_21');
                    }
                    fs.writeFileSync(buildGradlePath, buildGradle);
            ...
            if (fs.existsSync(rootBuildGradlePath)) {
                ...
                rootBuildGradle = rootBuildGradle.replace(/classpath 'com\.android\.tools\.build:gradle:\d+\.\d+\.\d+'/, "classpath 'com.android.tools.build:gradle:8.12.0'");
            ...
            if (fs.existsSync(gradleWrapperPropertiesPath)) {
                ...
                gradleWrapperProperties = gradleWrapperProperties.replace(/distributionUrl=.*gradle-[\d\.]+-all\.zip/, 'distributionUrl=https\\://services.gradle.org/distributions/gradle-8.13-all.zip');
...
async function copyAndroidIcons(cwd, config, logger) {
...
async function addPushNotificationsPlugin(cwd, logger) {
...
async function addDeploymentScripts(cwd, logger) {
