...
export const prepareIos = () => async ({ logger, config, cwd }) => {
...
async function addIosPlatform(cwd, logger) {
...
async function configureIosProject(cwd, config, logger) {
    const iosPath = path.join(cwd, 'ios');
    if (fs.existsSync(iosPath)) {
...
async function updateInfoPlist(iosPath, config, logger) {
    const infoPlistPath = path.join(iosPath, 'App/App/Info.plist');
    if (fs.existsSync(infoPlistPath)) {
        let infoPlist = fs.readFileSync(infoPlistPath, 'utf8');
        // Update bundle identifier
        infoPlist = infoPlist.replace(/<key>CFBundleIdentifier<\/key>\s*<string>.*?<\/string>/, `<key>CFBundleIdentifier</key><string>${config.appId}</string>`);
        // Update display name
        infoPlist = infoPlist.replace(/<key>CFBundleDisplayName<\/key>\s*<string>.*?<\/string>/, `<key>CFBundleDisplayName</key><string>${config.appName}</string>`);
        // Update version
        if (config.android.version) {
            infoPlist = infoPlist.replace(/<key>CFBundleShortVersionString<\/key>\s*<string>.*?<\/string>/, `<key>CFBundleShortVersionString</key><string>${config.android.version.name}</string>`);
            infoPlist = infoPlist.replace(/<key>CFBundleVersion<\/key>\s*<string>.*?<\/string>/, `<key>CFBundleVersion</key><string>${config.android.version.code}</string>`);
        }
        // Add URL schemes
        const urlSchemes = `
...
async function createEntitlements(iosPath, config, logger) {
...
async function updatePodfile(iosPath, config, logger) {
...
async function createXcconfigs(iosPath, config, logger) {
...
async function updateCapacitorConfig(cwd, config, logger) {
  ...
  ios: {
    ...
    scrollEnabled: true
            ...
            capacitorConfig = capacitorConfig.replace(/(\s+)(}\s*;?\s*$)/m, `$1${iosConfig}$1$2`);
            fs.writeFileSync(capacitorConfigPath, capacitorConfig);
            logger.debug('Updated Capacitor config with iOS settings');
        }
    }
}
async function generateIosAssets(cwd, config, logger) {
    ...
    if (!fs.existsSync(appIconPath)) {
        ...
        fs.writeFileSync(path.join(appIconPath, 'Contents.json'), JSON.stringify(contentsJson, null, 2));
...
async function createHandoffDocs(cwd, config, logger) {
